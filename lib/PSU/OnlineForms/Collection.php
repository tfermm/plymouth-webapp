...
abstract class Collection extends \PSU\Collection {
	...
	public function count() {
		...
		$count = \PSU::db('online_forms')->GetOne($sql);
		return $count;
	}//end count
	...
	public function get( $keys = null ) {
		...
		$rset = \PSU::db('online_forms')->Execute( $sql, $args );

		return $rset;
	}//end get

	public function get_by_active( $item, $it = null ) {
		return $this->get_by_filter( '\PSU\OnlineForms\FilterIterator\Active', $item, $it );
	}//end get_by_school
	...
	public function get_by_role( $item, $it = null ) {
		return $this->get_by_filter( '\PSU\OnlineForms\FilterIterator\Role', $item, $it );
	}//end get_by_school

	public function get_by_user( $item, $it = null ) {
		return $this->get_by_filter( '\PSU\OnlineForms\FilterIterator\User', $item, $it );
	}//end get_by_school
	...
	protected function _get_sql( $keys = null ) {
		...
		foreach( (array) $keys as $key => $value ) {
			...
			if( is_array( $value ) ) {
				...
				// simple scalar value
				$value = \PSU::db('online_forms')->qstr( $value );
				if( strpos( $key, '.' ) === false ) {
		...
		$sql = "
			...
			FROM online_forms.{$table} {$join_sql}
